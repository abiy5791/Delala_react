...
function ModalSearch({ id, searchId, modalOpen, setModalOpen }) {
  ...
  // live filter code starts
  const [search, setSearch] = useState("");
  const [alldata, setAlldata] = useState([]);

  const getUsersAndProps = async () => {
    const usersResponse = await axios.get("api/users");
    const propsResponse = await axios.get("api/prop");

    const mergedData = [...usersResponse.data, ...propsResponse.data];
                  ...
                  .filter((data) => {
                    return search.toLowerCase() === ""
                      ...
                      : data.title?.toLowerCase().includes(search) ||
                          ...
                          data.skills?.toLowerCase().includes(search) ||
                          ...
                          data.make?.toLowerCase().includes(search) ||
                          data.color?.toLowerCase().includes(search) ||
                          data.mileage?.toLowerCase().includes(search) ||
                          data.appropval
                            ?.toString()
                            .toLowerCase()
                            .includes(search) ||
                    ...
                    return (
                      <li>
                        <Link
                          ...
                          to={
                            data.model_type
                              ? `/admin_dashboard/${data.model_type}/${data.id}`
                        ...
                        >
                          ...
                          <span>
                            {data.model_type ? data.title : data.name}
                          </span>
                        </Link>
                      </li>
                    );
